 ...
 const fetchData = useCallback(async (weekId: string) => {
  ...
  const handleInputChange = (path: string, value: any, reorder = false) => {
    ...
    setData(prevData => {
        ...
        if (keys[0] === 'ventasWoman') {
            const tableKey = keys[1] as keyof WeeklyData['ventasWoman'];
             if (!updatedData.ventasWoman[tableKey]) updatedData.ventasWoman[tableKey] = [];
            const itemIndex = parseInt(keys[2], 10);
            const fieldKey = keys[3] as keyof VentasManItem;

            if (
                !isNaN(itemIndex) &&
                updatedData.ventasWoman &&
                Array.isArray(updatedData.ventasWoman[tableKey]) &&
